    ...
    for(float i = corner.y; i < u_Uvs.w; i+=u_pixelSize.y )
    {
			...
			if(dist < Tolerance)
      {
        ...
        return mix(texture2D(u_palTexture, vec2(testPos.x - u_pixelSize.x, testPos.y)), texture2D(u_palTexture, testPos), fract(u_paletteId));
	        ...
	        if (u_transmode == 1.0) {
          return Transparent; // Return Transparent if u_transmode equals 1
        } else if (u_transmode == 2.0 && inCol.a < 1.0) {
          return Transparent; // Return Transparent if u_transmode equals 2 and alpha < 1 (which isnt working GRAH)
        } else {
    return inCol;
		}
}
