function scr_player_handstandjump()
{
	...
	if scr_pizzyphys_check()
	{
		...
		{
			state = states.normal;
			movespeed = 0; // fix insta turn exploit
			if FINAL_GAMEPLAY && !grounded
			{
				image_index = 0;
		...
		if key_jump && grounded
		{
			...
			{
				scr_soundeffect(sfx_jump)
				if character == "SP" && global.newpizzy
				{
	...
	else
	{
		...
		if (animEnd or ((sprite_index == spr_suplexdashjump or 
		(sprite_index == spr_airattack && spr_airattack != spr_attackdash) 
		or (sprite_index == spr_suplexdashjumpstart && OLD_GAMEPLAY)) && !scr_slope())) 
		&& (grounded && vsp >= 0)
		{
		...
		if animEnd && sprite_index == spr_suplexdashjumpstart
			sprite_index = spr_suplexdashjump
		if animEnd && sprite_index == spr_airattackstart
			sprite_index = spr_airattack
		
		//Crouchslide
		if key_down && grounded && vsp >= 0 && character != "S" && character != "V"
		{
			grav = basegrav
			with instance_create(x, y, obj_jumpdust)
		        image_xscale = other.xscale
		...
		or sprite_index == spr_attackdash)
		{
			image_index = 0
		...
		//Noise Bomb
		if key_shoot2 && character == "N" && OLD_GAMEPLAY && !global.usebullet
		{
	...
	if bump && (slop or !grounded) && character != "S"
	&& FINAL_GAMEPLAY
	{
		if character == "N" && noisetype == 0
		{
		...
		else if !grounded or (!scr_solidwall(x, y - 32) or place_meeting(x, y - 32, obj_destructibles))
		{
			wallspeed = movespeed
	...
	// Bump
	if bump && state != states.climbwall && (!place_meeting(x + hsp, y, obj_shotgunblock) && character != "S")
	{
		if !ledge_bump() && (!place_meeting(x + xscale, y, obj_slope) or
		scr_solid_slope(x + xscale, y) or scr_solidwall(x + xscale, y) 
