function scr_player_bombgrab()
{
	...
	if (!instance_exists(bombgrabID))
		state = states.normal;
	if (sprite_index != spr_haulingstart && sprite_index != spr_uppercutfinishingblow 
...
function scr_player_bombpepfinal()
{
	alarm[8] = 60;
	alarm[7] = 120;
	hurted = true;
	...
	if (bombpeptimer == 0 && sprite_index == spr_bombpeprunabouttoexplode)
	{
		hurted = true;
		scr_losepoints();
		instance_create(x, y, obj_bombexplosionnew);
	...
	if (key_slap2 && state != states.normal && sprite_index != spr_bombpepintro && sprite_index != spr_bombpepend)
	{
		...
		with (instance_create(x + (xscale * 50), y, obj_pizzagoblinbombnew))
		{
			hsp = other.xscale * 8;
			vsp = -11;
			if (scr_solid(x, y))
