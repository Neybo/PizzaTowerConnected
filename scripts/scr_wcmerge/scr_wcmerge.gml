...
function scr_wc_step()
{
	...
	if WC_drag_toggle && WC_debug && WC_select_mode <= -1
	{
		...
		{
			// drag player
			if mb == 1 && keyboard_check(vk_control) && instance_exists(obj_player)
			{
				WC_drag_offset = [0, 0];
				WC_drag_alt = keyboard_check(vk_alt);
				WC_drag_inst = instance_find(obj_player, 0);
			}
			// normal drag
			else
			{
		...
		if instance_exists(WC_drag_inst)
		{
			...
			if !WC_drag_alt or rel
			{
				...
				{
					var frozen = WC_frozen[|i]
					if instance_exists(frozen[0]) && variable_instance_exists(frozen[0], "id") 
					&& frozen[0].id == WC_drag_inst.id
					{
						if frozen[1] == "x"
...
function scr_wc_draw()
{
	...
	if WC_debugview && instance_exists(WC_debugview_target)
	{
		...
		// draw the sprite
		if instance_exists(WC_debugview_target) && WC_debugview_target != global && !WC_debugview_target.visible or (variable_instance_exists(WC_debugview_target, "image_alpha") && WC_debugview_target.image_alpha <= 0)
			WCscr_drawobject(WC_debugview_target);
		
		// and draw the center of the object
		draw_set_colour(c_red);
		if (instance_exists(WC_debugview_target) && WC_debugview_target != global && variable_instance_exists(WC_debugview_target, "x") && variable_instance_exists(WC_debugview_target, "y")) 
...
function scr_wc_drawgui()
{
	...
	if WC_debugview
	{
		...
		draw_text(4, 24, "room: " + room_get_name(room) + " (" + string(room) + ")" +
		"\nfps: " + string(min(fps_real,60)) + " instances: " + string(instance_number(all)));
		...
		if instance_exists(WC_debugview_target) or WC_debugview_target == global
		{
			...
			else
			{
				...
				{
					if WC_debugview_target.object_index == obj_player
					&& string_copy(objvars[b], 1, 4) == "spr_" 
					{
						continue;
					}
					...
					draw_set_colour(c_white);
					if objvars[b] == "state"
						draw_set_color(c_admin)
					else if objvars[b] == "movespeed"
						draw_set_color(c_green)
					else if string_char_at(string(getvar), 1) == "-"
						draw_set_colour(merge_colour(c_white, c_red, 0.5));
					var todraw = string_replace_all(string(getvar), "\n", "\\n");
					
					if b <= 32 + WC_debugview_scroll
						draw_text(956, (yinc * 16) + 4, objvars[b] + ": " + todraw);
					yinc++;
				}
	...
	if instance_exists(WC_drag_inst)
	{
		...
		draw_text(guiwidth / 2, 0, dragtext + object_get_name(WC_drag_inst.object_index) + "\n" + postext);
	}
