function scr_player_grab()
{
	...
	if grounded
	{
		...
		if sprite_index != spr_swingding or OLD_GAMEPLAY
	...
	if key_shoot2 && REMIX_GAMEPLAY && baddiegrabbedID.object_index != obj_pizzaballOLD
	{
	...
	if swingdingbuffer > 0 && FINAL_GAMEPLAY
	{
		swingdingbuffer--;
		if movespeed > 0
			movespeed -= 0.1;
	...
	//Throws
	if key_slap2 && (sprite_index != spr_swingding or swingdingendcooldown > 20 or REMIX_GAMEPLAY)
	{
		if move != 0
			move = xscale
		...
		if sprite_index == spr_swingding
			...
			sprite_index = choose(spr_finishingblow1, spr_finishingblow2, 
			spr_finishingblow3, spr_finishingblow4, spr_finishingblow5)
		else if key_up
		...
		if (baddiegrabbedID == obj_otherplayer)
		{
			with (baddiegrabbedID)
				gms_p2p_send(p2p.pvp_grabkickarena, player_id)
			if (global.hp > 0)
				with (obj_pvparena)
					damagedealt += 1
		}
	...
	if abs(hsp) > 0 && grounded && sprite_index == spr_haulingwalk 
	&& (floor(image_index) == step1 or floor(image_index) == step2)
	{
		if !steppy
