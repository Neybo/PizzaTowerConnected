function scr_enemy_walk()
{
	...
	if !grounded &&	object_index != obj_noisey && object_index != obj_ancho 
	&& object_index != obj_kentukybomber && object_index != obj_miniufo// && object_index != obj_clownmato
	&& FINAL_GAMEPLAY
	{
	...
	or (sold && !inst_relation(sold, obj_slope))
	{
		...
		else if object_index == obj_indiancheese && REMIX_GAMEPLAY
		{
			image_xscale *= -1
			image_index = 0
	...
	if object_index != obj_ancho && object_index != obj_miniufo
	{
		...
		or place_meeting(x + hsp * 5, y + 30, obj_water)
		{
			...
			{
				if object_index == obj_ninja && OLD_GAMEPLAY
				{
					vsp = -11
