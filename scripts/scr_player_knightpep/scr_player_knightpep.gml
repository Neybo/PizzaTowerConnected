function scr_player_knightpep()
{
	...
	//Movement
	if (sprite_index = spr_knightpepwalk or sprite_index = spr_knightpepjump 
	or sprite_index = spr_knightpepfall or sprite_index = spr_knightpepidle 
	or (sprite_index == spr_knightpepland && FINAL_GAMEPLAY))
	{
	...
	else if grounded || sprite_index == spr_knightpepdownthrust
	{
		hsp = 0
		move = 0
	}
	...
	//Jump
	if ((input_buffer_jump < 8) && vsp > 0) 
	&& grounded && (sprite_index == spr_knightpepidle 
	or sprite_index == spr_knightpepwalk or (sprite_index == spr_knightpepland && FINAL_GAMEPLAY)) 
	{
		input_buffer_jump = 8
	...
	&& sprite_index != spr_knightpepdoublejump && sprite_index != spr_knightpepdownthrust)
		sprite_index = spr_knightpepfall

	if (!grounded && (key_jump || input_buffer_jump < 8) && FINAL_GAMEPLAY) 
	&& !doublejump && sprite_index != spr_knightpepdownthrust
	{
		vsp = -11;
		image_index = 0;
		doublejump = true
	...
	//Land
	if (sprite_index == spr_knightpepfall 
	|| sprite_index == spr_knightpepdoublejump || sprite_index == spr_knightpepdownthrust)
	&& grounded && vsp > 0
	{
		with obj_baddie
	...
	if sprite_index == spr_knightpepdoublejump
	{
	...
	&& (floor(image_index) = 4 or floor(image_index) = 10)
