function scr_player_grabbed()
{
	...
	catch (e)
	{
		e = 0
		state = states.normal;
		exit; // no,no,non,on,non,non,on,no,non,on,
	}
	...
	with (player)
	{
		...
		try
		{
			hackyfix = !((state == states.grab) or (state == states.mach3) or (state == states.finishingblow) 
			or (state == states.grabbing) or (state == states._throw) or (state == states.slam) or (state == states.tacklecharge) 
			or (state == states.punch) or (state == states.superslam) or (state == states.backkick) or (state == states.uppunch) or (state == states.shoulder))
